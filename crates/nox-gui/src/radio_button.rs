        ...
        Style: WindowStyle<FontHash>,
{
    ...
    ) -> Vec2
    {
        ...
        self.size = vec2(diameter + style.item_pad_outer().x + text_size.x, max_y);
        self.size
    }
    ...
    ) -> UpdateResult
    {
        ...
        collect_text(
            &self.label_text,
            self.offset + vec2(diameter + style.item_pad_outer().x, size.y * 0.5 - text_height * 0.5),
            bounded_instance,
        );
        UpdateResult {
    ...
    )
    {
        ...
        set_vertex_params(vertices, self.handle_vertex_range, offset, target_color);
        if self.held() {
            target_color = style.active_widget_outline_col();
        ...
        if self.hovered() {
            ...
            set_vertex_params(vertices, self.focused_handle_outline_vertex_range, offset, target_color);
        } else {
            hide_vertices(vertices, self.focused_handle_outline_vertex_range);
        }
        if self.selected() {
            target_color =
                if self.held() || self.hovered() {
                    style.focused_text_col()
                } else {
                    style.text_col()
                };

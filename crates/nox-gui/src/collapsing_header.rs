...
impl CollapsingHeader {
    ...
    )
    {
        if self.label != label {
            ...
            self.label_text = text_renderer.render(
                &[text_segment(&self.label, &style.regular_font)], false, 0.0 
            ).unwrap_or_default();
        }
    }
    ...
    ) -> f32
    {
        ...
        or_flag!(
            self.flags,
            Self::HOVERED,
            bounding_rect.is_point_inside(cached_data.cursor_pos) && !widget_active
        );
        if !widget_active && self.hovered() && cached_data.mouse_button_left_state.pressed() {
            self.flags ^= Self::COLLAPSED;
        }
        if self.collapsed() {
            self.rotation =
                (self.rotation - FRAC_PI_2 * style.animation_speed * cached_data.delta_time)
                .clamp(0.0, FRAC_PI_2);
        } else {
            self.rotation =
                (self.rotation + FRAC_PI_2 * style.animation_speed * cached_data.delta_time)
    ...
    pub fn set_vertex_params(&self, style: &UiStyle, vertices: &mut [Vertex]) {

...
impl<'a> CommandScheduler<'a> {
            ...
            for<'b, 'c> Cmd::Target<'b, 'c>: Commands<'b, 'c>
    {
        ...
        let f = super::command::make_fn_record_command(move |rec, id, alloc| {
            let mut command = Cmd::new(rec, id, queue.take().unwrap()).context_with(|| format_compact!(
        ...
        let fp = unsafe {
            let ptr = self.inner.get_mut().stack.alloc_uninit(1)?;
            ptr.write(f);
            ptr
        };
...
impl CommandRecorderInner<'_> {
            ...
            Alloc: LocalAlloc<Error = Error>,
    {
        ...
        let mut submits = Submits {
            submits: FixedVec32::with_capacity(sorted.len() + 2, alloc)?,
            present_submits: FixedVec32::new(alloc),
        };
        let mut global_wait_semaphore_infos = NonNullVec32::with_capacity(sorted.len() + 1, alloc)?;
        ...
        if !self.surfaces.is_empty() {
            ...
            let mut submit = SubmitInfo {
                ...
                command_buffer_infos: NonNullVec32::with_capacity(1, alloc)?,
            ...
            let mut all_signal_semaphores = FixedVec32::with_capacity(n_present_submits, alloc)?;
            submits.present_submits = FixedVec32::with_capacity(n_present_submits, alloc)?;
            for (queue, swapchains) in &self.cache.get_mut().present_submits.submits {
                if swapchains.is_empty() {
                    continue
                }
                let n_swapchains = swapchains.len();
                let mut wait_semaphores = FixedVec32::with_capacity(n_swapchains, alloc)?;
                let mut handles = FixedVec32::with_capacity(n_swapchains, alloc)?;
                let mut indices = FixedVec32::with_capacity(n_swapchains, alloc)?;
                let mut present_id2 = FixedVec32::with_capacity(n_swapchains, alloc)?;
                let mut signal_semaphores = FixedVec32::with_capacity(n_swapchains, alloc)?;
        ...
        log::debug!("queue scheduler stack used {} bytes", self.inner.stack.used());
        unsafe {

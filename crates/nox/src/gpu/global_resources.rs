...
use nox_mem::{
    ...
    vec_types::{FixedVec, GlobalVec, ArrayVec, Vector},
    Allocator, AsRaw,
};

use crate::dev::{
    error::{Error, Result, Context, location, ErrorContext},
    has_bits,
};

use crate::gpu::{*, memory_binder::{MemoryBinder, DeviceMemory, MemoryBinderError}};

use super::{PhysicalDeviceInfo, LinearDeviceAlloc};
...
impl GlobalResources {
    ...
    ) -> Result<Self>
    {
        ...
        let dummy_layout = unsafe { RaiiHandle::new(
            device
                ...
                .context_with(|| format_compact!("unexpected vulkan error at {}", location!()))?,
    ...
    #[inline(always)]
    pub(crate) fn get_shader(&self, id: ShaderId) -> Result<&Shader> {
        self.shaders
            .get(id.0)
            .context("failed to find shader")
    }
    ...
    ) -> Result<PipelineLayoutId>
    {
        ...
        for id in shaders {
            s.push(self.shaders.get(id.0)
                .context("couldn't find shader")?
            ).ok();
        }
    ...
    #[inline(always)]
    pub(crate) fn get_pipeline_layout(&self, id: PipelineLayoutId) -> Result<&PipelineLayout> {
    ...
    {
        let mut writes = FixedVec
            ::with_capacity(image_updates.len() + buffer_updates.len(), alloc)
            .context("vec error")?;
        let mut image_infos = FixedVec
            ::with_capacity(image_updates.len(), alloc)
            .context("vec error")?;
        for update in image_updates {
            let set = self.shader_resources
                .get_mut(update.resource.0)
                .context_with(|| "couldn't find shader resource")?;
            ...
            let Some(ty) = self.pipeline_layouts
                ...
                return Err(Error::just_context("invalid shader resource image binding {} of set {}"))
            };
            let mut vk_infos = FixedVec
                ::with_capacity(update.infos.len(), alloc)
                .context("vec error")?;
            for info in update.infos {
                let (id, range_info) = info.image_source;
                let sampler = self.samplers
                    .get(info.sampler.0)
                    .context("couldn't find sampler")?
                    .handle;
                let image_view =
                    if let Some(range_info) = range_info {
                        ...
                        let set = self.shader_resources
                            .get_mut(update.resource.0)
                            .context("couldn't find shader resource for image update")?;
        ...
        let mut buffer_infos = FixedVec
            ::with_capacity(buffer_updates.len(), alloc)
            .context("vec error")?;
        for update in buffer_updates {
            let set = self.shader_resources
                .get(update.resource.0)
                .context("couldn't find shader resource for buffer update")?;
            let vk_set = set.descriptor_set;
            let Some(ty) = self.pipeline_layouts
                .get(set.layout_id.0)
                .context("couldn't find pipeline layout")?
                ...
                return Err(Error::just_context(format_compact!(
                    "invalid shader resource image binding {} of set {}",
                    update.binding, set.set,
                )))
            };
            let mut vk_infos = FixedVec
                ::with_capacity(update.infos.len(), alloc)
            ...
            for info in update.infos {
                let buffer = self.buffers
                    .get(info.buffer.0)
                    .context("couldn't find buffer")?;
                let properties = buffer.properties();
                if info.offset + info.size > properties.size {
                    return Err(Error::new("buffer error", BufferError::OutOfRange {
        ...
        let mut vk_copies = FixedVec
            ::with_capacity(copies.len(), alloc)
            .context("vec error")?;
        for copy in copies {
            let src = self.shader_resources
                .get(copy.src_resource.0)
                .context("couldn't find source shader resource for copy")?;
            let dst = self.shader_resources
                .get(copy.dst_resource.0)
                .context("couldn't find destination shader resource for copy")?;
            if copy.src_binding >= src.binding_count {
                return Err(Error::just_context(format_compact!(
                    "shader resource copy source binding {} was out of range with source binding count {}",
                    copy.src_binding, src.binding_count,
                )))
            }
            if copy.dst_binding >= dst.binding_count {
                return Err(Error::just_context(format_compact!(
                    "shader resource copy destination binding {} was out of range with destination binding count {}",
                    copy.src_binding, dst.binding_count,
                )))
            }
    ...
    ) -> Result<GlobalVec<u8>>
    {
        ...
        unsafe {
            ...
            if result != vk::Result::SUCCESS {
                return Err(Error::new("failed to get pipeline cache data", result))
            }
    ...
    ) -> Result<()>
    {
        ...
        let mut create_infos = FixedVec
            ::with_capacity(pipeline_count, alloc)
            .context("vec error")?;
        let mut vk_infos = FixedVec
            ::with_capacity(pipeline_count, alloc)
            .context("vec error")?;
        for info in infos {
            let info = create_infos.push(
                info.as_create_info(&self, alloc)
        ...
        unsafe {
            ...
            if result != vk::Result::SUCCESS {
                return Err(Error::new("failed to create graphics pipelines", result))
            }
    ...
    ) -> Result<()>
    {
        ...
        let mut vk_infos = FixedVec
            ...
            .context("vec error")?;
        for info in infos {
            vk_infos.push(info.as_create_info(self).context("failed to convert compute pipeline info")?).unwrap();
        }
        let mut pipelines = FixedVec
        ...
        unsafe {
            ...
            if result != vk::Result::SUCCESS {
                return Err(Error::new("failed to create compute pipelines", result))
            }
            ...
            F: FnMut(u32) -> ShaderResourceId,
    {
        ...
        let mut res = FixedVec
            ::with_capacity(sets.len(), alloc)
            .context(ErrorContext::VecError(location!()))?;
        for (i, set) in sets.iter().enumerate() {
            if set.0.len() != 0 {
                let resource = self.shader_resources
                    .get(f(i as u32).0)
                    .context("couldn't find shader resource")?;
            ...
            F: FnMut(PushConstant) -> &'b [u8],
    {
        ...
        let mut res = FixedVec
            ::with_capacity(push_constants.len(), alloc)
            .context(ErrorContext::VecError(location!()))?;
    ...
    pub(crate) fn get_graphics_pipeline(&self, id: GraphicsPipelineId) -> Result<&GraphicsPipeline> {
        self.graphics_pipelines
            .get(id.0)
            .context("couldn't find graphics pipeline")
    }
    ...
    pub(crate) fn get_compute_pipeline(&self, id: ComputePipelineId) -> Result<&ComputePipeline> {
        self.compute_pipelines
            .get(id.0)
            .context("couldn't find compute pipeline")
    }
    ...
    {
        if size == 0 {
            return Err(Error::new("buffer size was zero", MemoryBinderError::ZeroSizeAlloc))
        }
        ...
        unsafe {
            match binder {
                ResourceBinderBuffer::DefaultBinder => {
                    buffer.set_memory(
                        self.default_binder
                            .bind_buffer_memory(buffer.handle(), None)
                            .context("failed to bind buffer memory")?
                    );
                },
                ResourceBinderBuffer::DefaultBinderMappable => {
                    buffer.set_memory(self.default_binder_mappable
                        .bind_buffer_memory(buffer.handle(), None)
                        .context("faileld to bind buffer memory")?
                    );
                },
                ResourceBinderBuffer::LinearDeviceAlloc(id) => {
                    let mut alloc = self.linear_device_allocs
                        .get_mut(id.0).context("couldn't find linear device alloc")?
                        .write().unwrap();
    ...
    #[inline(always)]
    pub(crate) fn get_buffer(&self, id: BufferId) -> Result<&Buffer> {
        self.buffers
            .get(id.0)
            .context("failed to find buffer")
    }

    #[inline(always)]
    pub(crate) fn get_buffer_mut(&mut self, id: BufferId) -> Result<&mut Buffer> {
        self.buffers
            .get_mut(id.0)
            .context("failed to find buffer")
    }
        ...
        unsafe {
            match binder {
                ResourceBinderImage::DefaultBinder => {
                    image.set_memory(self.default_binder
                        .bind_image_memory(image.handle(), None)
                        .context("failed to bind image memory")?
                    );
                },
                ResourceBinderImage::DefaultBinderMappable => {
                    image.set_memory(self.default_binder_mappable
                        .bind_image_memory(image.handle(), None)
                        .context("failed to bind image memory")?
                    );
                }
                ...
                ResourceBinderImage::Owned(b, fallback) => {
                    image.set_memory(b
                        .bind_image_memory(image.handle(), fallback)
                        .context("failed to bind image memory")?
                    );
                },
            };
    ...
    pub fn lock_linear_device_alloc(
        ...
        let alloc = self.linear_device_allocs
            .get_mut(id.0)
            .context("couldn't find linear device alloc")?;
        let mut write = alloc.write().unwrap();
        if write.semaphore_count != 0 {
            return Err(Error::just_context("linear device alloc was already locked"))
        }
    ...
    /// Waits for previous semaphores untill `timeout` where `timeout` is in nanoseconds.
    ...
    pub fn wait_for_semaphores(
        ...
        let mut handles = FixedVec
            ::with_capacity(semaphores.len(), alloc)
            .context("vec error")?;
        let mut values = FixedVec
            ::with_capacity(semaphores.len(), alloc)
            .context("vec error")?;
        let mut unlock_resources = FixedVec
            ::with_capacity(semaphores.len(), alloc)
            .context("vec error")?;
        for &(id, value) in semaphores {
            let semaphore = self.timeline_semaphores
                .get_mut(id.0)
                .context("couldn't find timeline semaphore")?;
            handles.push(semaphore.handle).ok();
            values.push(value).ok();
            let unlock_resources = unlock_resources.push(FixedVec
                ::with_capacity(semaphore.locked_resources.len(), alloc)
                .context("vec error")?
            ).unwrap();
